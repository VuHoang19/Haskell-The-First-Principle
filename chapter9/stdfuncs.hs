...
-- myMaximumBy takes a comparison function and a list and returns the greatest element of the list 
-- based on the last value that the comparision returned GT for.
myMaximumBy :: (a -> a -> Ordering) -> [a] -> a 
myMaximumBy f (x:[]) = x
myMaximumBy f (x:xs) = case f x mnx of 
                         LT -> mxs 
                         EQ -> x 
                         GT -> x
                        where mxs = myMaximumBy f xs 

-- myMinimumBy takes comparision function and a list and returns the least element of the list
-- based on the last value that the comparison returned LT for.
...
-- Using the myMinimumBy and myMaximumBy functions, write yor own versions of maximum and minimum.
myMaximum :: (Ord a) => [a] -> a
